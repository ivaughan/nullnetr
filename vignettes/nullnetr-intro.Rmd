...
1. `WelshStreams`. Part of the macroinvertebrate food web from upland streams in south Wales, UK (Pearson *et al*., submitted). The network focuses on two abundant predators (*Rhyacophila dorsalis* and *Dinocras cephalotes*), with 85 individuals and 16 potential prey. Distinguished from a bipartite network by intra-guild predation between the two predators.
...
par(mfrow = c(1, 2))
plot_preferences(stream.1, "Rhyacophila", signif.level = 0.95, 
                 bar.colours = c("#67a9cf", "#f7f7f7", "#ef8a62"), bar.type = 
                 "counts", ylab = "Num. of prey detections", res.order = 
                 WelshStreams.order, cex.names = .7)
plot_preferences(stream.1, "Dinocras", signif.level = 0.95, 
                 bar.colours = c("#67a9cf", "#f7f7f7", "#ef8a62"), bar.type = 
                 "counts", ylab = "Num. of prey detections", res.order = 
                 WelshStreams.order, cex.names = .7)
```
...
PlotWebByLevel(BS, link.colour.by = "Test", link.colour.spec = c(Stronger = 
               ...
               label.colour = "white")
legend("topright", legend = c("Stronger", "ns", "Weaker"), lty = 1, lwd = 2, col = 
       c("#d7191c", "#cccccc", "#2c7bb6"))
```
...
plot_preferences(bs.null, "Trissopelopia", signif.level = 0.95, bar.type = 
                 "counts", bar.colours = c("#67a9cf", "#f7f7f7", "#ef8a62"),
                 ylab = "Number of visits", res.order =  
                 data.frame(Broadstone.nodes$node, Broadstone.nodes$M))
plot_preferences(bs.null, "Cordulegaster", signif.level = 0.95,  bar.type = 
                 "counts",  bar.colours = c("#67a9cf", "#f7f7f7", "#ef8a62"),
                 ylab = "Number of visits", res.order = 
                 data.frame(Broadstone.nodes$node, Broadstone.nodes$M))
```
...
Pearson, C.E., Symondson, W.O.C., Clare, E.L., Ormerod, S.J., Bolanos, I.E. and Vaughan, I.P. (submitted) The effects of pastoral intensification on the feeding interactions of generalist predators in streams.
